...
> 完整 17 项核对清单、不匹配清单模板、鉴权矩阵、禁止事项等见 [integration-test.md §11](integration-test.md#11-前后端接口契约一致性审查api-contract-review补充-17-项)。
>
> 本节是 PR 合并前的**强制门禁**，前后端联调前必查。

### 7.1 必查 6 项（每项都通过才能合并）

- [ ] **数量一致**：后端 `@router.get/post/...` 数量 == 前端 `api/*.ts` 中 export 函数数量（±0，禁止多调或死接口）
- [ ] **路径 + 方法一致**：HTTP 方法、URL、前后缀斜杠完全匹配（错配 404/405）
- [ ] **请求参数一致**：Path / Query / Body 字段名、类型、必填、枚举值一一对应（**禁止** 前端 `pageNum` 调 Python snake_case 后端）
- [ ] **响应结构一致**：`R[T]` 的 `code` / `message` / `data` / `trace_id` 三端对齐（FastAPI 成功 `code=0`），空值/大数字序列化策略一致
- [ ] **鉴权一致**：每个接口的 `Depends(require_role)` / `Depends(require_permission)` 与前端 `meta.roles` 字符串完全一致（区分大小写）
- [ ] **越权必须 403**：越权访问（USER 调 ADMIN 接口）必须返回 `code=403` 而非 404（避免信息泄露）

### 7.2 快速核对（接口清单收集）

```bash
# 1. 后端 FastAPI / Flask 接口清单
